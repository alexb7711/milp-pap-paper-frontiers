...
\begin{abstract}
  ...
  \hl{To model the BEB discharges over their respective routes, it is assumed each BEB has an average kWh charge loss
    while on route.} The optimization coordinates BEB charging to ensure that each vehicle \hl{remains above a specified
    state-of-charge (SOC)}. The model also minimizes the total number of chargers utilized and prioritizes slow
  \hl{charging} for battery health. The model validity is demonstrated with a set of routes sampled from the Utah
