...
Constraint \ref{subeq:space} is nearly identical to \eqref{subeq:bapspace}, but rather than viewing the charger as a
continuous strip of length $S$, it is discretized into $n_Q$ queues a width of unit length one. A BEB is also assigned a
unit length of one which is reflected in \eqref{subeq:space} by $\cdot \geq 1$. Constraint \ref{subeq:valid_depart} ensures
that the time the BEB is detatched from the charger, $c_i$, is before its departure time, $\tau_i$. Constraint \ref{subeq:vspace}
...
pairing. The last constraints \eqref{subeq:binaryspace}-\eqref{subeq:Ispace} define the sets of valid values for each
variable.
