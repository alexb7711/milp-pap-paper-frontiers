...
map a visit index to a bus index with the shorthand $\Gamma_i$ used to refer to the bus index $b$ for visit $i$.}}
...
Most variables are now defined in terms of a visit. src_latex{\hl{A singular visit is defined by the following steps:
the BEB arrives at the station, is assigned a queue with a specified duration, then departs for its next route.}} Two
...
that $n_Q = n_B + n_C$, where $n_B$ is the number of buses and $n_C$ is the number of chargers. src_latex{\hl{The
